...
float mAx, mAy, mAz;
//таблица значений дл€ корректировки ускорений
...
/**----------------------------------------------------------------------------------------------------*/
/**
  * @brief 			“аск расчЄта углов и ускорений + семафорим дл€ записи на —ƒ карту
  * @reval			None
  */
void IMU_Calcualte(void)
{
	...
	if(uIMUCounter < IMU_LOW_DATA_SIZE)
	{
		...
		accumData[uIMUCounter].fLatitude = 0;	//заглушка числового значени€ широты
		accumData[uIMUCounter].fLongitude = 0;	//заглушка числового значени€ долготы
