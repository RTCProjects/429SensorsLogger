...
#define RADAR_RX_PACK_SIZE			7	//размер пакета данных в сообщениях радара
#define RADAR_RX_BUFFER_QUERY_SIZE	1	//размер очереди DMA буфера
#define RADAR_RX_DATA_QUERY_SIZE	8	//размер очереди для данных расстояния
...
/**
 * Структура принимаемого пакета данных от радара в общем виде
 */
typedef struct
{
	uint16_t	start_seq;
...
/**
 * Структура отправляемого в очередь для СКИФ сообщения с данными расстояния
 */
typedef struct
{
	uint16_t	distance;
...
}radar_target_into_bytefield_t;	//структура битового поля [40:47] сообщения 0x70C
typedef struct
{
	uint8_t		index;
	uint8_t		rcs;
...
typedef struct
{
	uint8_t		rollcount:2;
	uint8_t		rsvd:6;
}radar_target_status_bytefield_t; //структура битового поля [8:9] сообщения 0x70B
typedef struct
{
	uint8_t		nooftarget;
